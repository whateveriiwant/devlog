...
- 작성일: 2026-09-26
- 상태: GitHub Actions 배포 적용 완료, D1 시험 이전 검증 완료, 런타임 전환 진행 중
...
1. **Staging 공개 글 상세 연결·검증 (완료)** — 사이트 Worker가 공개 글 요청을 staging CMS/D1에서 조회하고 미리 빌드한 Astro 레이아웃에 넣어 응답한다. 본문·날짜·SEO 메타데이터·목차·태그·시리즈·이전/다음 링크와 R2 이미지 주소를 표본 글에서 확인했다. 없는 글은 404, 내부 템플릿 URL은 외부에서 404, `/write`는 로그인으로 리다이렉트된다. 응답은 `no-store`다. 프로덕션은 변경하지 않았다.
2. **목록·검색·분류·RSS·사이트맵 읽기 연결 (staging 완료)** — `/blog/` 목록·커서 페이지네이션, 시리즈 및 태그 목록·상세, 검색 UI, RSS, sitemap을 D1 공개 데이터로 연결했다. 새 분류는 공통 템플릿으로 제공하도록 구현했다. staging에서 표본 응답을 검증했으며 프로덕션은 변경하지 않았다. Worker CPU 표본이 무료 10ms 한도에 가까워 프로덕션 전환 전 추가 성능 검토가 필요하다.
3. **편집기 쓰기 경로 전환 (staging 기본 흐름 완료)** — 허용 계정 로그인, 초안 저장, 발행, 기존 글 수정 중 공개본 보존, 재출간 revision 및 날짜 갱신, 휴지통 및 복구, 이미지 업로드와 초안 이미지 참조 저장을 확인했다. 빠른 더블클릭 저장이 1회만 반영되고 동일 request ID를 재전송했을 때 `{duplicate:true}` 응답과 D1 revision 불변을 확인했다. 오래된 다중 탭 저장은 거절된다. 10만 자 초과 본문은 브라우저 미리보기를 건너뛰며, staging에서 31만 자 초안의 저장과 렌더 HTML 크기 제한에 따른 발행 거절을 확인했다. 초안은 남고 공개본은 추가되지 않았다. 렌더러 예외(422) 자체를 강제로 만드는 검증은 남았다. 프로덕션에는 배포하지 않았다.
4. **삭제·복구·이미지 정리 전환 (staging 유예·재참조 확인, 7일 만료 삭제 대기)** — staging 전용 버킷·D1 조합으로 미참조 마커 생성과 재참조 취소를 검증했다. 실제 7일 경과 삭제는 아직 대기 중이다. 정리 스크립트와 Actions는 D1·Git 참조 합산으로 수정했다. 저장소 배포 토큰의 D1 조회 권한과 운영 dry-run은 병합 후 확인해야 한다.
5. **백업 및 롤백 검증 (스냅샷 복원 완료)** — staging과 운영 D1 백업을 각각 로컬 SQLite에 복원해 행 수를 확인했다. D1 새 글·휴지통 상태를 Markdown으로 내보내고 정적 사이트로 되돌리는 검증은 실제 D1 쓰기 전환 뒤 남는다.
6. **프로덕션 전환** — D1 조회 권한, 이미지 정리 dry-run, 실제 7일 경과 삭제, D1에서 Markdown으로 내보내는 롤백을 확인한 뒤 CMS·사이트를 전환한다. 성공 응답 후 글 상세·목록·검색 반영도 운영에서 확인한다.
