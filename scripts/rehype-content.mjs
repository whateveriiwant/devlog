...
const posts = JSON.parse(
  fs.readFileSync(
    new URL('../migration/velog-posts.json', import.meta.url),
    'utf8'
  )
);
...
const paths = new Map(
  posts.map((p) => [
    decodeURIComponent(new URL(p.originalUrl).pathname),
...
export function rehypeContent() {
  ...
  if (origin && new URL(origin).protocol !== 'https:')
    throw new Error('MEDIA_BASE_URL must be HTTPS');
  return (tree) => {
    let hasTitleHeading = false;
    visit(tree, 'element', (node) => {
      if (node.tagName === 'h1') hasTitleHeading = true;
    });
    visit(tree, 'element', (node) => {
      if (hasTitleHeading && /^h[1-5]$/.test(node.tagName))
        node.tagName = `h${Number(node.tagName[1]) + 1}`;
      if (node.tagName === 'img') {
        const source = String(node.properties.src || '');
        if (origin && assets.has(source))
          node.properties.src = `${origin}/${assets.get(source)}`;
      ...
      if (node.tagName === 'a' && node.properties.href) {
        try {
          ...
          const local =
            target.hostname === 'velog.io' &&
            paths.get(decodeURIComponent(target.pathname.replace(/\/$/, '')));
          if (local) node.properties.href = local + target.search + target.hash;
        } catch {
          /* Keep malformed source links for the review report. */
        }
      }
    });
  };
