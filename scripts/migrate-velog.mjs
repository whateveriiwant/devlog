...
const previous = fs.existsSync(path.join(root, 'migration/file-manifest.json'))
  ? read('migration/file-manifest.json')
  : [];
const memberships = new Map();
for (const s of series)
  for (const p of s.posts) {
    if (memberships.has(p.id))
      throw new Error(`Multiple series need review: ${p.id}`);
    memberships.set(p.id, { id: s.id, order: p.order });
  }
const planned = posts.map((post) => {
  if (!post.isMarkdown || hash(post.content) !== post.contentSha256)
    throw new Error(`Source format/hash needs review: ${post.title}`);
  const metadata = {
    title: post.title,
...
for (const file of planned) {
  ...
  if (current !== file.content && (!last || hash(current) !== last.sha256))
    throw new Error(`Local edit protected: ${file.path}`);
}
for (const file of planned) {
...
fs.writeFileSync(
  path.join(root, 'migration/file-manifest.json'),
  JSON.stringify(
    planned.map((f) => ({
      path: f.path,
      sourceId: f.sourceId,
...
console.log(
  `Migrated ${posts.length} posts and ${series.length} series. Body bytes unchanged.`
);
