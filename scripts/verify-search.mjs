...
globalThis.document = { currentScript: null, querySelector: () => ({ getAttribute: () => 'ko' }) };
globalThis.fetch = async input => {
  const file = fileURLToPath(input instanceof Request ? input.url : input);
  assert.ok(file.startsWith(bundle + path.sep), 'Search fetch stays inside the built bundle');
  return new Response(await fs.readFile(file));
};
const pagefind = await import(pathToFileURL(path.join(bundle, 'pagefind.js')).href);
await pagefind.options({ basePath: pathToFileURL(bundle + path.sep).href, baseUrl: '/' });
const all = await pagefind.search(null);
assert.equal(all.results.length, posts.length, 'Every migrated article is searchable');
const documents = await Promise.all(all.results.map(hit => hit.data()));
for (const post of posts) {
  const doc = documents.find(doc => decodeURIComponent(doc.url) === `/blog/${post.slug}/`);
...
for (const [query, expectedSlug] of [
  ...
  assert.ok(data.some(doc => decodeURIComponent(doc.url) === `/blog/${expectedSlug}/`), `Query: ${query}`);
