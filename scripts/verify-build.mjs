...
const posts = JSON.parse(
  fs.readFileSync(path.join(root, 'migration/velog-posts.json'), 'utf8')
);
const series = JSON.parse(
  fs.readFileSync(path.join(root, 'migration/velog-series.json'), 'utf8')
);
const assets = new Map(
  JSON.parse(
    fs.readFileSync(path.join(root, 'migration/image-manifest.json'), 'utf8')
  ).map((image) => [image.originalUrl, image.key])
);
...
const cache = new Map();
const failures = [];
const sourceLinkWarnings = [];
const treeFor = (file) => {
  if (!cache.has(file))
    cache.set(file, parser.parse(fs.readFileSync(file, 'utf8')));
...
const signature = (tree) => ({
  headings: elements(tree, (n) => /^h[1-6]$/.test(n.tagName)).map((n) => ({
    depth: Number(n.tagName[1]),
    text: plain(toString(n)),
  })),
  images: elements(tree, (n) => n.tagName === 'img').map((n) =>
    String(n.properties.src)
  ),
...
for (const post of posts) {
  try {
    ...
    if (expected.headings.some((h) => h.depth === 1))
      expected.headings = expected.headings.map((h) => ({
        ...h,
        depth: Math.min(6, h.depth + 1),
      }));
    if (mediaOrigin)
      expected.images = expected.images.map((src) =>
        assets.has(src) ? `${mediaOrigin}/${assets.get(src)}` : src
      );
    assert.deepEqual(
      actual.headings,
      expected.headings,
      'Rendered headings and text'
    );
    assert.deepEqual(
      actual.images,
      expected.images,
      'Rendered image count and mapped source URL'
    );
    ...
    assert.deepEqual(
      actual.code.map(withoutTerminalLF),
      expected.code.map(withoutTerminalLF),
      'Rendered code, excluding terminal LF only'
    );
    terminalNewlineDifferences += actual.code.filter(
      (code, i) => code !== expected.code[i]
    ).length;
    assert.equal(
      elements(tree, (n) => Object.hasOwn(n.properties, 'dataPagefindBody'))
        .length,
      1,
      'Single searchable article'
    );
    assert.equal(
      elements(
        tree,
        (n) =>
          n.tagName === 'time' && n.properties.dateTime === post.publishedAt
      ).length,
      1,
    ...
    assert.equal(
      elements(prose, (n) =>
        ['script', 'iframe', 'object', 'embed'].includes(n.tagName)
      ).length,
      0,
    ...
    assert.equal(
      elements(prose, (n) =>
        Object.keys(n.properties).some((k) => /^on[A-Za-z]/.test(k))
      ).length,
      0,
      'No raw event handlers'
    );
    const ids = elements(tree, (n) => typeof n.properties.id === 'string').map(
      (n) => n.properties.id
    );
    assert.equal(ids.length, new Set(ids).size, 'Unique element IDs');
    images += actual.images.length;
    headings += actual.headings.length;
    blocks += actual.code.length;
  } catch (error) {
    failures.push({ post: post.title, error: error.message });
  }
}
const htmlFiles = fs
...
for (const file of htmlFiles) {
  ...
  const originalLinks = new Set(
    prose
      ? elements(prose, (n) => n.tagName === 'a').map((n) => n.properties.href)
      : []
  );
  for (const link of elements(
    tree,
    (n) => n.tagName === 'a' && n.properties.href
  )) {
    try {
      ...
      assert.ok(
        target.startsWith(dist + path.sep) || target === dist,
        'Local path remains under dist'
      );
      ...
      if (url.hash && target.endsWith('.html')) {
        ...
        assert.ok(
          elements(treeFor(target), (n) => n.properties.id === id).length,
          `Missing fragment ${url.pathname}${url.hash}`
        );
      }
      checkedLinks++;
    } catch (error) {
      ...
      if (originalLinks.has(link.properties.href))
        sourceLinkWarnings.push(record);
...
for (const s of series) {
  ...
  const links = elements(
    main,
    (n) => n.tagName === 'a' && hasClass(n, 'post-row')
  ).map((n) => n.properties.href);
  try {
    assert.deepEqual(
      links,
      s.posts.map((p) => `/blog/${encodeURIComponent(p.slug)}/`)
    );
  } catch (error) {
    failures.push({ series: s.name, error: 'Rendered order mismatch' });
  }
}
const result = {
...
fs.writeFileSync(
  path.join(root, 'migration/build-verification.json'),
