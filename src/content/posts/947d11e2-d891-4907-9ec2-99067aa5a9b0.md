---
title: Storybook 알아보기
description: 프론트엔드 테스트 도구 중 하나인 Storybook에 대해
slug: Storybook-알아보기
publishedAt: 2025-10-31T02:05:38.720Z
updatedAt: 2026-09-06T08:23:43.558Z
series:
  id: 928b6b91-8174-41a7-a245-d3a65755410d
  order: 1
tags:
  - React
  - storybook
  - 테스트
draft: false
originalUrl: https://velog.io/@jsj9620/Storybook-%EC%95%8C%EC%95%84%EB%B3%B4%EA%B8%B0
thumbnail: https://velog.velcdn.com/images/jsj9620/post/8382fe43-b76f-4c30-a554-df03b0a76def/image.png
---
![](https://velog.velcdn.com/images/jsj9620/post/8382fe43-b76f-4c30-a554-df03b0a76def/image.png)

# Storybook

Storybook은 리액트 컴포넌트를 앱의 전체적인 맥락(라우팅, API 통신 등)과 완전히 분리하여, 독립적인 UI 개발 환경에서 구축하고 시각적으로 테스트할 수 있게 해주는 도구이다.

단위 테스트가 컴포넌트의 **로직**을 분리하여 테스트한다면, Storybook은 컴포넌트의 **시각적 결과물**을 분리하여 개발하고 검토한다고 생각하면 쉽다.
...
스토리는 컴포넌트의 **특정 시각적 상태**를 캡처한 것이다.
...
![](https://i.ibb.co/nMwSyY9C/image.png)

작성하고 `npm run storybook`으로 실행하면 Storybook UI를 실행할 수 있다.
...
- 방법: Storybook의 `play` 함수와 `@storybook/test-runner`를 사용
- 동작: 컴포넌트 스토리 파일을 수정
...
- 실행:
  ...
  2. CI (Headless): `npm run test-storybook` 명령어를 실행하면, `@storybook/test-runner`가 모든 스토리의 `play` 함수를 실제 브라우저(Headless) 환경에서 실행하여 Vitest처럼 테스트 리포트를 출력

# 요약

정리하자면, Storybook은 단순히 컴포넌트를 모아보는 카탈로그가 아니라, 다음과 같은 역할을 하는 **중심적인 테스트 허브**이다.
