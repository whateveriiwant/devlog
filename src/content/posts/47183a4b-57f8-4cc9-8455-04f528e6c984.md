---
title: Node.js Passport 라이브러리
description: Node.js를 위한 인증 미들웨어인 Passport 라이브러리에 대한 정리와 JWT와의 사용법까지
slug: Node.js-Passport-라이브러리
publishedAt: 2025-09-03T11:55:55.879Z
updatedAt: 2026-09-07T04:11:18.060Z
category: web-app
series:
...
tags:
  ...
  - 인증
draft: false
originalUrl: https://velog.io/@jsj9620/Node.js-Passport-%EB%9D%BC%EC%9D%B4%EB%B8%8C%EB%9F%AC%EB%A6%AC
thumbnail: https://velog.velcdn.com/images/jsj9620/post/0c40431c-333c-4bb8-b776-9b3acaf4105e/image.png
---
![](https://velog.velcdn.com/images/jsj9620/post/0c40431c-333c-4bb8-b776-9b3acaf4105e/image.png)
# Passport
...
Passport의 가장 중요한 특징은 **전략**을 사용한다.

로그인을 하는 방식은 매우 다양하다.

ID와 비밀번호를 사용할 수도 있고, 구글, 네이버 같은 소셜 계정으로 로그인할 수도 있다.

Passport는 이러한 각각의 인증 방식을 **전략**이라는 이름의 독립적인 모듈로 분리한다.
...
5. 후처리
   - 성공 시: `passport.authenticate()`는 `next()`를 호출하여 다음 미들웨어(라우터의 콜백 함수)로 제어권을 넘긴다.
...
이 방식은 서버가 사용자의 로그인 상태를 **기억**해야 한다.

1. 최초 로그인: 사용자가 아이디/비밀번호를 제출하면 `passport-local`이 이 DB와 대조하여 확인
...
1. 최초 로그인: 사용자가 아이디/비밀번호를 제출하면 `passport-local`이 이 DB와 대조하여 확인
2. JWT 발급: 인증에 성공하면, 서버는 세션을 만드는 대신 **사용자 정보가 담긴 JWT를 발급**해서 브라우저에 전송
