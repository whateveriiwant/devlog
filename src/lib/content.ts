...
export const tagsFor = (post: Post) => [...new Set(post.data.tags.map(normalizeTag))];
export const postUrl = (post: Post) => `/blog/${encodeURIComponent(post.data.slug)}/`;
export const seriesUrl = (slug: string) => `/series/${encodeURIComponent(slug)}/`;
export const tagUrl = (tag: string) => `/tags/${encodeURIComponent(tagSlug(tag))}/`;
export const dateKey = (date: Date) => new Intl.DateTimeFormat('sv-SE', { timeZone: 'Asia/Seoul' }).format(date);
export const dateLabel = (date: Date) => dateKey(date).replaceAll('-', '.');
export const readMinutes = (post: Post) => Math.max(1, Math.ceil((post.body || '').replace(/```[\s\S]*?```/g, '').replace(/https?:\/\/\S+/g, '').length / 700));
