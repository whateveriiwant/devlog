          ...
          >
            ...
            >
              ...
              <ItemContent className="min-w-0 gap-1.5">
                ...
                <ItemTitle className="block text-sm leading-6 font-medium wrap-anywhere">
                  {row.title}
                </ItemTitle>
                {!compact && (
                  <ItemDescription className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                    ...
                    <span>{row.tags.slice(0, 3).join(' · ')}</span>
                    {numbered && <time dateTime={row.iso}>{row.date}</time>}
                  </ItemDescription>
                )}
              </ItemContent>
