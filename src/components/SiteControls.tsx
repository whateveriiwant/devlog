...
export default function SiteControls({ sections }: { sections: NavSection[] }) {
  ...
  const searchFocus = useRef<HTMLElement | null>(null),
    imageFocus = useRef<HTMLElement | null>(null);
  useEffect(() => {
    ...
    const key = (event: KeyboardEvent) => {
      if (
        ...
        if (!document.querySelector('[role="dialog"]')) {
          searchFocus.current = document.activeElement as HTMLElement;
          setOpen(true);
        }
      }
    };
    document.querySelectorAll<HTMLImageElement>('.prose img').forEach((img) => {
      ...
      button.setAttribute(
        'aria-label',
        img.alt ? `${img.alt} 확대 보기` : '이미지 확대 보기'
      );
      img.before(button);
      button.append(img);
    });
    ...
    <div className="ml-auto flex shrink-0 items-center gap-1.5">
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button
            variant="outline"
            className="mr-1 h-8 w-9 px-0 text-muted-foreground sm:w-44 sm:justify-start sm:px-3"
            aria-label="전체 글 검색"
            onClick={() => {
          ...
          >
            <Search />
            <span className="hidden sm:inline">글 검색</span>
        ...
        >
          ...
          <DialogDescription className="sr-only">
            제목과 본문에서 검색합니다. 화살표 키로 결과를 선택하고 Enter로 열
            수 있습니다.
          </DialogDescription>
            ...
            <CommandList className="max-h-[55dvh] px-2 pb-2">
              {results.map((data) => (
                <CommandItem
                  ...
                  value={data.url}
                  asChild
                  onSelect={() => location.assign(data.url)}
                      ...
                      {data.meta.publishedAt && (
                        <time dateTime={data.meta.publishedAt}>
                          {new Intl.DateTimeFormat('sv-SE', {
                            timeZone: 'Asia/Seoul',
                          }).format(new Date(data.meta.publishedAt))}
                        </time>
                      )}
                    </span>
      ...
      <Sheet open={menu} onOpenChange={setMenu}>
        ...
        <SheetContent side="left" className="gap-0">
          <SheetHeader className="border-b p-5">
            <SheetTitle>seungjun.dev</SheetTitle>
            <SheetDescription>주제와 시리즈로 기록 찾기</SheetDescription>
          </SheetHeader>
      ...
      <Dialog
        open={Boolean(photo)}
        onOpenChange={(value) => {
          if (!value) setPhoto(null);
        }}
      >
        <DialogContent
          className="max-h-[95dvh] overflow-auto sm:max-w-[90vw]"
          onCloseAutoFocus={(event) => {
            event.preventDefault();
            imageFocus.current?.focus();
          }}
        >
          ...
          {photo && (
            <img
              src={photo.src}
              alt={photo.alt}
              className="mx-auto max-h-[78dvh] max-w-full rounded-md object-contain"
            />
          )}
          <DialogDescription
            className={photo?.caption ? 'text-center' : 'sr-only'}
          >
            {photo?.caption || '본문 이미지의 확대 보기입니다.'}
          </DialogDescription>
        </DialogContent>
      </Dialog>
    </div>
