...
const stars = [
  [12, 19, 2.1], [25, 12, 2.8], [37, 23, 1.7], [52, 11, 2.9],
  [17, 32, 1.7], [31, 34, 2.1], [45, 31, 2.3], [59, 33, 1.8],
  [8, 35, 1], [23, 26, .9], [43, 18, .9],
];
...
export default function ThemeSwitch({ dark, onToggle }: { dark: boolean; onToggle: () => void }) {
  ...
  return (
    <button
      ...
      onClick={() => { setJourney(value => value + 1); onToggle(); }}
    >
      <span className="theme-switch__scene" aria-hidden="true">
        <span className="theme-switch__halos"><i /><i /><i /></span>
        <svg className="theme-switch__stars" viewBox="0 0 120 45" fill="none">
          {stars.map(([x, y, r], index) => (
            <path key={index} style={{ '--star-delay': `${index * 35}ms` } as CSSProperties}
              d={`M ${x} ${y-r} Q ${x+.18*r} ${y-.18*r} ${x+r} ${y} Q ${x+.18*r} ${y+.18*r} ${x} ${y+r} Q ${x-.18*r} ${y+.18*r} ${x-r} ${y} Q ${x-.18*r} ${y-.18*r} ${x} ${y-r} Z`} />
          ))}
        </svg>
        <svg className="theme-switch__clouds theme-switch__clouds--back" viewBox="0 0 120 45">
        ...
        <svg className="theme-switch__clouds theme-switch__clouds--front" viewBox="0 0 120 45">
        ...
        <span className="theme-switch__travellers" key={journey}>
          <svg className="theme-switch__balloon" viewBox="0 0 24 36">
            <path fill="#cb383d" d="M12 1C-3 1-2 17 5 23L9 27H15L19 23C26 17 27 1 12 1Z" />
            <path fill="#f3a630" d="M12 1C4 1 5 18 10 26H14C19 18 20 1 12 1Z" />
            <path fill="#f6d665" d="M12 2C10 7 10 20 12 26C14 20 14 7 12 2Z" />
            <path stroke="#6d4146" strokeWidth=".8" d="M8 25L9 31M16 25L15 31" />
            <rect x="8" y="30" width="8" height="5" rx="1.5" fill="#175274" />
            <rect x="7.5" y="29.5" width="9" height="1.6" rx=".8" fill="#286787" />
          </svg>
          <svg className="theme-switch__rocket" viewBox="0 0 24 40">
            <path className="theme-switch__flame" fill="#ff933d" d="M8 26Q5 33 12 40Q19 33 16 26Z" />
            <path fill="#ffe980" d="M10 26Q9 33 12 36Q15 33 14 26Z" />
            <path fill="#b92f46" d="M7 18Q0 20 1 30L8 27M17 18Q24 20 23 30L16 27" />
        ...
        <span className="theme-switch__orb">
          ...
          <span className="theme-switch__moon"><i /><i /><i /></span>
